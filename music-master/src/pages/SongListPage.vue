  ...
  <div class="table">
    <div class="container">
      <div class="handle-box">
        ...
        <el-button type="primary" size="mini" class="handle-del mr10" @click="delAllSongList" icon="el-icon-delete">批量删除</el-button>
        <el-button type="primary" size="mini" @click="centerDialogVisible = true" icon="el-icon-circle-plus-outline">添加歌单</el-button>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      addSongListUrl: 'songList/add',
      delSongListUrl: 'songList/delete',
      updateSongListUrl: 'songList/update'
    }
  },
  computed: {
    // 计算当前表格中的数据
  ...
  methods: {
    ...
    saveEdit () {
      ...
      this.postData(params, this.updateSongListUrl)
        .then(res => {
          if (res.data.code === 1) {
    ...
    addsongList () {
      ...
      params.append('pic', '/img/songListPic/123.jpeg')
      params.append('introduction', this.registerForm.introduction)
      params.append('style', this.registerForm.style)
      this.postData(params, this.addSongListUrl)
        .then(res => {
          if (res.data.code === 1) {
            this.getSongListData()
            this.registerForm = {}
          ...
          console.log('addsongList', err)
        })
      this.centerDialogVisible = false
    },
    // 确定删除
    deleteRow (ids) {
      this.getData({id: ids}, this.delSongListUrl)
        .then(res => {
          if (res) {
            this.getSongListData()
